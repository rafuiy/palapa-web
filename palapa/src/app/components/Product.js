        ...
        {cards.map((card, index) => (
          <SwiperSlide key={index}>
            <div className="bg-white shadow-lg rounded-lg p-6">
              <Image src={card.image} alt={card.title} className="w-full h-22 p-3 object-contain shadow-[30px_0px_50px_rgba(80,110,255,0.3)] rounded-md" />
              <h2 className="mt-4 text-lg font-bold">{card.title}</h2>
              <p className="text-gray-600 text-sm">{card.description}</p>
              <button 
                onClick={() => setActiveModal(card.key)} 
                className="mt-5 px-4 py-1 bg-blue-500 text-xs text-white rounded-lg"
              >
                Lihat lebih detail
              </button>
            </div>
